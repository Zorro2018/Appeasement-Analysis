...
def perturb_gmv_row(row, rng):
    ...
    for k in ("stddev_delta_gmv", "stderr_delta_gmv"):
        if k in row:
            out[k] = as_str_like(row[k], abs(round(num(row[k]) * jit(rng, 0.9, 1.1), 2)))
...
def perturb_missing_order(row, rng):
    ...
    new_incident_count = max(1, round(new_order_count * ratio * jit(rng, 0.95, 1.05)))
    ...
    new_qty = round(num(row["avg_qty_adjusted"]) * jit(rng, 0.9, 1.1), 2)
    new_lines = round(num(row["avg_total_lines"]) * jit(rng, 0.9, 1.1), 2)
    new_survey = max(1, round(num(row["survey_responses"]) * jit(rng)))
    new_csat_pct = min(99.5, max(50.0, num(row["csat_pct"]) * jit(rng, 0.95, 1.05)))
...
NARRATIVE_FIXES = [
    ("A $25 promo on a $117 order is a 21% discount", "A $25 promo on a $132 order is a 19% discount"),
    ("At $10\\u2013$15 (8\\u201313% of AOV)", "At $10\\u2013$15 (8\\u201311% of AOV)"),
    ("This range delivers 23\\u201325pp of lift vs no promo.", "This range delivers 19\\u201321pp of lift vs no promo."),
    ("based on the 1.8pp advantage $15 holds over $25", "based on the 1.5pp advantage $15 holds over $25"),
    ("Control group (13.7M) dwarfs promo groups (170K\\u2013995K)", "Control group (12.1M) dwarfs promo groups (145K\\u2013846K)"),
    ("single-digit declines (7\\u201312%)", "single-digit declines (6\\u201310%)"),
    ("much steeper declines (26\\u201337%)", "much steeper declines (22\\u201331%)"),
    ("as $25 (only ~4pp difference vs ~29pp for Non-W+)", "as $25 (only ~3pp difference vs ~25pp for Non-W+)"),
]
